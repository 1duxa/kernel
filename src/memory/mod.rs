...
pub unsafe fn init(boot_info: &BootInfo) -> Result<(), &'static str> {
        ...
        if region.kind == MemoryRegionKind::Usable {
            ...
            if region.start >= 0x100000 && region.start < 0x1000000 {
                ...
                if usable_end > usable_start
                    && (usable_end - usable_start) > (frame_end - frame_start)
                {
    ...
    if frame_start == 0 {
        ...
        println!(
            "INIT: WARNING - Using fallback frame region {:#x}-{:#x}",
            frame_start, frame_end
        );
    }

    println!(
        "INIT: Kernel at ~0x1000000. Frame allocator: {:#x}-{:#x}",
        frame_start, frame_end
    );
    ...
    println!(
        "INIT: Frame allocator: start={:#x}, end={:#x}",
        frame_start, frame_end
    );
